...
public class LaneDetect {
    ...
    private Mat lane_line_markings;
    ...
    public void laneDetection() {
        ...
        this.lane_line_markings = get_line_markings(this.orig_frame);
        // Perform the perspective transform to generate a bird's eye view
        Mat warped_frame = perspective_transform(this.lane_line_markings, roi_points, desired_roi_points);
...
//        System.out.println("transformation_matrix:"+transformation_matrix.size());
    ...
    private void get_lane_line_indices_sliding_windows(Mat warped_frame) {
        ...
        System.out.println("nonzerox" + nonzerox.get(nonzerox.size() - 1));
...
//            Imgproc.rectangle(frame_sliding_window, rect_right, new Scalar(255, 255, 255), 2);
            ...
            if (good_left_inds.size() > this.minpix) {
                leftx_current = average(nonzerox.subList(good_left_inds.get(0), good_left_inds.get(good_left_inds.size() - 1)));
            }
            if (good_right_inds.size() > this.minpix) {
                rightx_current = average(nonzerox.subList(good_right_inds.get(0), good_right_inds.get(good_right_inds.size() - 1)));
            }
        }
